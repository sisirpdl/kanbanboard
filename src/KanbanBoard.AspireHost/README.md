...
1. Set `KanbanBoard.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../KanbanBoard.Infrastructure/KanbanBoard.Infrastructure.csproj -s KanbanBoard.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../KanbanBoard.Infrastructure/KanbanBoard.Infrastructure.csproj -s KanbanBoard.Web.csproj
```
